...
public class FollowingServiceTest {
    ...
    public void setup() throws IOException, TweeterRemoteException {
        ...
        validRequest = new FollowingRequest(currentUser.getAlias(), 3, null, "fakeToken");
        invalidRequest = new FollowingRequest(null, 0, null, "fakeToken");

        // Setup a mock ServerFacade that will return known responses
        successResponse = new FollowingResponse(Arrays.asList(resultUser1, resultUser2, resultUser3), false);
        ServerFacade mockServerFacade = Mockito.mock(ServerFacade.class);
        Mockito.when(mockServerFacade.getFollowees(validRequest, FollowingService.URL_PATH)).thenReturn(successResponse);

        failureResponse = new FollowingResponse("An exception occurred");
        Mockito.when(mockServerFacade.getFollowees(invalidRequest, FollowingService.URL_PATH)).thenReturn(failureResponse);
